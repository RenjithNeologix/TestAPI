...
class EAPI {
    ...
    public function sendRequest($request, $parameters = array()) {
        ...
        if (!$this->url OR ! $this->clientCode OR ! $this->username OR ! $this->password) {
            ...
            __('Missing parameters', self::MISSING_PARAMETERS)
            );
        }
        ...
        if ($error)
            throw new \Magento\Framework\Exception\LocalizedException(
            __('CURL error: ' . $response . ':' . $error . ': ' . $errorNumber, self::CURL_ERROR)
            );
    ...
    protected function getSessionKey() {
        ...
        if (!isset($_SESSION))
            throw new \Magento\Framework\Exception\LocalizedException(
            __('PHP session not started', self::PHP_SESSION_NOT_STARTED)
            );
        ...
        if (
            ...
            if (!isset($response['records'][0]['sessionKey'])) {
                ...
                $e = new \Magento\Framework\Exception\LocalizedException(
                        __('Verify user failure', self::VERIFY_USER_FAILURE)
                );
                $e->response = $response;
                throw $e;
            }
